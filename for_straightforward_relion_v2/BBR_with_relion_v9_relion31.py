...
def main():
	(inlst,x_coord,y_coord,z_coord,output,debug_,MNX,MNY,SYMMETRY) =  parse_command_line()
	sym_num=0
	...
	mline=judge_mline1(inline=inlst_line,start=MLINE)
	print "Mline="+str(MLINE)+" mline="+str(mline)
	for i in range(0,mline):
		r.write(str(inlst_line[i]))
	LAST_METADATA_NUM=int(inlst_line[mline-1].split()[1].split('#')[1])
	r.write("_rlnDeltaZAngst #"+str(LAST_METADATA_NUM+1)+"\n")
	r.write("_rlnParticleSerialNumber #"+str(LAST_METADATA_NUM+2)+"\n")
	
	for i in range(0,MLINE):
		if (inlst_line[i].split()):
			if (str(inlst_line[i].split()[0])=="_rlnImagePixelSize"):
				APIX_index=int(inlst_line[i].split('#')[1])-1
			if (str(inlst_line[i].split()[0])=="_rlnImageSize"):
				NY_index=int(inlst_line[i].split('#')[1])-1
	apix=float(inlst_line[MLINE].split()[APIX_index])
	NY=float(inlst_line[MLINE].split()[NY_index])
	print "apix="+str(apix)+" NY="+str(NY)
	for i in range(MLINE,mline):
		if (inlst_line[i].split()):
			...
			if (str(inlst_line[i].split()[0])=="_rlnOriginXAngst"):
				OX_index=int(inlst_line[i].split('#')[1])-1
			if (str(inlst_line[i].split()[0])=="_rlnOriginYAngst"):
...
#	for i in range(3,4):
		...
		#	df_old=(dfu+dfv)/2.0
			...
			ox=float(inlst_line[i].split()[OX_index])/apix
			oy=float(inlst_line[i].split()[OY_index])/apix
	...
	#		NEED rot=0,180,0 here if full-particles match I3 sym
			...
			(center1_eman,center2_eman)=CENTER_EMAN(ox,oy,NY)
			...
			for k in range (0,sym_num):
				...
				new_x=icos_mult_mx0*(x_coord-NY/2)+icos_mult_mx1*(y_coord-NY/2)+icos_mult_mx2*(z_coord-NY/2)
				new_y=icos_mult_mx3*(x_coord-NY/2)+icos_mult_mx4*(y_coord-NY/2)+icos_mult_mx5*(z_coord-NY/2)
				new_z=icos_mult_mx6*(x_coord-NY/2)+icos_mult_mx7*(y_coord-NY/2)+icos_mult_mx8*(z_coord-NY/2)
		...
		#			print "here "+str(dfu)+" "+str(new_dfu)
				...
				tmpx=crdx-new_ox
				tmpy=crdy-new_oy
				if(tmpx<0.0 or tmpx>MNX or tmpy<0.0 or tmpy>MNY):
				...
				for j in range(0,xx):
					...
					if(j==PSI_index):
						...
						tmp+=str(new_psi)
					if(j==OX_index):
						tmp+=str(round(new_ox)*apix)
					if(j==OY_index):
						tmp+=str(round(new_oy)*apix)
					if(j==crdX_index):
						tmp+=str(crdx)
					if(j==crdY_index):
						tmp+=str(crdy)
...
def parse_command_line():
	usage="%prog <input star> <point x> <point y> <point z> <output> <DEBUG, plus=1, minus=0, nochange=2> <micrograph nx> <micrograph ny> <Symmetry>\nSupported symmetry is Cn, Dn, O and I3"
	parser = OptionParser(usage=usage, version="%1")
	
	if len(sys.argv)<10: 
		print "<input star> <point x> <point y> <point z> <output> <DEBUG, plus=1, minus=0, nochange=2> <micrograph nx> <micrograph ny> <Symmetry>\nSupported symmetry is Cn, Dn, O and I3"
		sys.exit(-1)
...
#	mline=int(args[1])
	...
	MNY=int(args[7])
	SYMMETRY=str(args[8])
	return (inlst,x_coord,y_coord,z_coord,output,debug_,MNX,MNY,SYMMETRY)
